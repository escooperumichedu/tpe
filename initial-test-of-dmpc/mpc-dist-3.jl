...
N = 300  # Control horizon
dt = 1 # Sampling time of the system
Nd = 10
dtd = 30

global cpu_max = 15.0 # Maximum cpu time for Ipopt
...
global c_cpu_max = 15.0 # Maximum cpu time for Ipopt
...
function init_sp_and_params()
    ...
    global T1_init = 473.9984192821725
    global T2_init = 450.0724004578051
    global T3_init = 464.92665118171556
    global T4_init = 465.0512540934039
    global T5_init = 468.1656192706613

    global V1_init = 1.0
    global V2_init = 0.9869545002999998
    global V3_init = 0.9999999997000003
    global V4_init = 3.000000000030002
    global V5_init = 1.0

    global CA1_init = 9102.493105830325
    global CA2_init = 7718.265898515155
    global CA3_init = 6199.419067323682
    global CA4_init = 1760.861556781438
    global CA5_init = 5816.371917740881

    global CB1_init = 22.401870118838513
    global CB2_init = 16.876376053977673
    global CB3_init = 25.438549705103245
    global CB4_init = 14.042848028637229
    global CB5_init = 4.593733579295451

    global CC1_init = 1115.79975452567
    global CC2_init = 1773.1410468179108
    global CC3_init = 2585.7901261605775
    global CC4_init = 5402.340484347595
    global CC5_init = 3713.3137135903594

    global CD1_init = 220.2116493047642
    global CD2_init = 335.1873401633306
    global CD3_init = 497.3431706634123
    global CD4_init = 735.8081484179255
    global CD5_init = 204.22013863743646
...
function cmpc()
    ...
    JuMP.@variables mpc begin
        ...
        # Volume, state, [=] m3
        V1[k=0:N], (lower_bound=min(V1_init, V1_sp) * 0.8, upper_bound=max(V1_init, V1_sp) * 1.2)
        V2[k=0:N], (lower_bound=min(V2_init, V2_sp) * 0.8, upper_bound=max(V2_init, V2_sp) * 1.2)

        # Temperature, state, [=] K
        T1[k=0:N], (lower_bound=min(T1_init, T1_sp) * 0.8, upper_bound=max(T1_init, T1_sp) * 1.2)
        T2[k=0:N], (lower_bound=min(T2_init, T2_sp) * 0.8, upper_bound=max(T2_init, T2_sp) * 1.2)

        # Concentration, state, [=] mol/m3
        CA1[k=0:N], (lower_bound=min(CA1_init, CA1_sp) * 0.8, upper_bound=max(CA1_init, CA1_sp) * 1.2)
        CA2[k=0:N], (lower_bound=min(CA2_init, CA2_sp) * 0.8, upper_bound=max(CA2_init, CA2_sp) * 1.2)

        CB1[k=0:N], (lower_bound=min(CB1_init, CB1_sp) * 0.8, upper_bound=max(CB1_init, CB1_sp) * 1.2)
        CB2[k=0:N], (lower_bound=min(CB2_init, CB2_sp) * 0.8, upper_bound=max(CB2_init, CB2_sp) * 1.2)

        CC1[k=0:N], (lower_bound=min(CC1_init, CC1_sp) * 0.8, upper_bound=max(CC1_init, CC1_sp) * 1.2)
        CC2[k=0:N], (lower_bound=min(CC2_init, CC2_sp) * 0.8, upper_bound=max(CC2_init, CC2_sp) * 1.2)

        CD1[k=0:N], (lower_bound=min(CD1_init, CD1_sp) * 0.8, upper_bound=max(CD1_init, CD1_sp) * 1.2)
        CD2[k=0:N], (lower_bound=min(CD2_init, CD2_sp) * 0.8, upper_bound=max(CD2_init, CD2_sp) * 1.2)

        # Volume, state, [=] m3    
        V3[k=0:N], (lower_bound=min(V3_init, V3_sp) * 0.8, upper_bound=max(V3_init, V3_sp) * 1.2)

        # Temperature, state, [=] K
        T3[k=0:N], (lower_bound=min(T3_init, T3_sp) * 0.8, upper_bound=max(T3_init, T3_sp) * 1.2)

        # Concentration, state, [=] mol/m3
        CA3[k=0:N], (lower_bound=min(CA3_init, CA3_sp) * 0.8, upper_bound=max(CA3_init, CA3_sp) * 1.2)

        CB3[k=0:N], (lower_bound=min(CB3_init, CB3_sp) * 0.8, upper_bound=max(CB3_init, CB3_sp) * 1.2)

        CC3[k=0:N], (lower_bound=min(CC3_init, CC3_sp) * 0.8, upper_bound=max(CC3_init, CC3_sp) * 1.2)

        CD3[k=0:N], (lower_bound=min(CD3_init, CD3_sp) * 0.8, upper_bound=max(CD3_init, CD3_sp) * 1.2)

        V4[k=0:N], (lower_bound=min(V4_init, V4_sp) * 0.8, upper_bound=max(V4_init, V4_sp) * 1.2)
        V5[k=0:N], (lower_bound=min(V5_init, V5_sp) * 0.8, upper_bound=max(V5_init, V5_sp) * 1.2)

        # Temperature, state, [=] K
        T4[k=0:N], (lower_bound=min(T4_init, T4_sp) * 0.8, upper_bound=max(T4_init, T4_sp) * 1.2)
        T5[k=0:N], (lower_bound=min(T5_init, T5_sp) * 0.8, upper_bound=max(T5_init, T5_sp) * 1.2)

        # Concentration, state, [=] mol/m3
        CA4[k=0:N], (lower_bound=min(CA4_init, CA4_sp) * 0.8, upper_bound=max(CA4_init, CA4_sp) * 1.2)
        CA5[k=0:N], (lower_bound=min(CA5_init, CA5_sp) * 0.8, upper_bound=max(CA5_init, CA5_sp) * 1.2)

        CB4[k=0:N], (lower_bound=min(CB4_init, CB4_sp) * 0.8, upper_bound=max(CB4_init, CB4_sp) * 1.2)
        CB5[k=0:N], (lower_bound=min(CB5_init, CB5_sp) * 0.8, upper_bound=max(CB5_init, CB5_sp) * 1.2)

        CC4[k=0:N], (lower_bound=min(CC4_init, CC4_sp) * 0.8, upper_bound=max(CC4_init, CC4_sp) * 1.2)
        CC5[k=0:N], (lower_bound=min(CC5_init, CC5_sp) * 0.8, upper_bound=max(CC5_init, CC5_sp) * 1.2)

        CD4[k=0:N], (lower_bound=min(CD4_init, CD4_sp) * 0.8, upper_bound=max(CD4_init, CD4_sp) * 1.2)
        CD5[k=0:N], (lower_bound=min(CD5_init, CD5_sp) * 0.8, upper_bound=max(CD5_init, CD5_sp) * 1.2)
...
function dmpc_1_arc1()

    # Control variables: F1, F2, F3, Q1
    # State variables: All state variables associated with CSTR-1
    ...
    JuMP.@variables mpc begin
        ...
        # Volume, state, [=] m3
        V1[k=0:Nd], (lower_bound=min(V1_init, V1_sp) * 0.8, upper_bound=max(V1_init, V1_sp) * 1.2)

        # Temperature, state, [=] K
        T1[k=0:Nd], (lower_bound=min(T1_init, T1_sp) * 0.8, upper_bound=max(T1_init, T1_sp) * 1.2)

        # Concentration, state, [=] mol/m3
        CA1[k=0:Nd], (lower_bound=min(CA1_init, CA1_sp) * 0.8, upper_bound=max(CA1_init, CA1_sp) * 1.2)

        CB1[k=0:Nd], (lower_bound=min(CB1_init, CB1_sp) * 0.8, upper_bound=max(CB1_init, CB1_sp) * 1.2)

        CC1[k=0:Nd], (lower_bound=min(CC1_init, CC1_sp) * 0.8, upper_bound=max(CC1_init, CC1_sp) * 1.2)

        CD1[k=0:Nd], (lower_bound=min(CD1_init, CD1_sp) * 0.8, upper_bound=max(CD1_init, CD1_sp) * 1.2)
    ...
    @NLobjective(mpc, Min, sum(
        ...
        for k = 0:Nd) +
                           sum(
        w.f1 * (F1[k] - F1_sp)^2 + w.f2 * (F2[k] - F2_sp)^2 + w.f3 * (F3[k] - F3_sp)^2 +
        w.q1 * (Q1[k] - Q1_sp)^2 for k = 0:Nd
    )
    )
    ...
    return F1, F2, F3, Q1, V1, T1, CA1, CB1, CC1, CD1
end

function dmpc_2_arc1()
    ...
    JuMP.@variables mpc begin
        ...
        # Volume, state, [=] m3
        V2[k=0:Nd], (lower_bound=min(V2_init, V2_sp) * 0.8, upper_bound=max(V2_init, V2_sp) * 1.2)
        V3[k=0:Nd], (lower_bound=min(V3_init, V3_sp) * 0.8, upper_bound=max(V3_init, V3_sp) * 1.2)
        V4[k=0:Nd], (lower_bound=min(V4_init, V4_sp) * 0.8, upper_bound=max(V4_init, V4_sp) * 1.2)
        V5[k=0:Nd], (lower_bound=min(V5_init, V5_sp) * 0.8, upper_bound=max(V5_init, V5_sp) * 1.2)

        T2[k=0:Nd], (lower_bound=min(T2_init, T2_sp) * 0.8, upper_bound=max(T2_init, T2_sp) * 1.2)
        T3[k=0:Nd], (lower_bound=min(T3_init, T3_sp) * 0.8, upper_bound=max(T3_init, T3_sp) * 1.2)
        T4[k=0:Nd], (lower_bound=min(T4_init, T4_sp) * 0.8, upper_bound=max(T4_init, T4_sp) * 1.2)
        T5[k=0:Nd], (lower_bound=min(T5_init, T5_sp) * 0.8, upper_bound=max(T5_init, T5_sp) * 1.2)

        CA2[k=0:Nd], (lower_bound=min(CA2_init, CA2_sp) * 0.8, upper_bound=max(CA2_init, CA2_sp) * 1.2)
        CA3[k=0:Nd], (lower_bound=min(CA3_init, CA3_sp) * 0.8, upper_bound=max(CA3_init, CA3_sp) * 1.2)
        CA4[k=0:Nd], (lower_bound=min(CA4_init, CA4_sp) * 0.8, upper_bound=max(CA4_init, CA4_sp) * 1.2)
        CA5[k=0:Nd], (lower_bound=min(CA5_init, CA5_sp) * 0.8, upper_bound=max(CA5_init, CA5_sp) * 1.2)

        CB2[k=0:Nd], (lower_bound=min(CB2_init, CB2_sp) * 0.8, upper_bound=max(CB2_init, CB2_sp) * 1.2)
        CB3[k=0:Nd], (lower_bound=min(CB3_init, CB3_sp) * 0.8, upper_bound=max(CB3_init, CB3_sp) * 1.2)
        CB4[k=0:Nd], (lower_bound=min(CB4_init, CB4_sp) * 0.8, upper_bound=max(CB4_init, CB4_sp) * 1.2)
        CB5[k=0:Nd], (lower_bound=min(CB5_init, CB5_sp) * 0.8, upper_bound=max(CB5_init, CB5_sp) * 1.2)

        CC2[k=0:Nd], (lower_bound=min(CC2_init, CC2_sp) * 0.8, upper_bound=max(CC2_init, CC2_sp) * 1.2)
        CC3[k=0:Nd], (lower_bound=min(CC3_init, CC3_sp) * 0.8, upper_bound=max(CC3_init, CC3_sp) * 1.2)
        CC4[k=0:Nd], (lower_bound=min(CC4_init, CC4_sp) * 0.8, upper_bound=max(CC4_init, CC4_sp) * 1.2)
        CC5[k=0:Nd], (lower_bound=min(CC5_init, CC5_sp) * 0.8, upper_bound=max(CC5_init, CC5_sp) * 1.2)

        CD2[k=0:Nd], (lower_bound=min(CD2_init, CD2_sp) * 0.8, upper_bound=max(CD2_init, CD2_sp) * 1.2)
        CD3[k=0:Nd], (lower_bound=min(CD3_init, CD3_sp) * 0.8, upper_bound=max(CD3_init, CD3_sp) * 1.2)
        CD4[k=0:Nd], (lower_bound=min(CD4_init, CD4_sp) * 0.8, upper_bound=max(CD4_init, CD4_sp) * 1.2)
        CD5[k=0:Nd], (lower_bound=min(CD5_init, CD5_sp) * 0.8, upper_bound=max(CD5_init, CD5_sp) * 1.2)
    ...
    for k = 0:Nd
        ...
        set_start_value(Q3[k], fix.Q3[k+1])
        set_start_value(Q4[k], fix.Q4[k+1])
        set_start_value(Q5[k], fix.Q5[k+1])
    end
    ...
    @constraints mpc begin
        ...
        CA2_inital, CA2[0] == CA2_init
        CA3_inital, CA3[0] == CA3_init
        CA4_inital, CA4[0] == CA4_init
        CA5_inital, CA5[0] == CA5_init

        CB2_inital, CB2[0] == CB2_init
        CB3_inital, CB3[0] == CB3_init
        CB4_inital, CB4[0] == CB4_init
        CB5_inital, CB5[0] == CB5_init

        CC2_inital, CC2[0] == CC2_init
        CC3_inital, CC3[0] == CC3_init
        CC4_inital, CC4[0] == CC4_init
        CC5_inital, CC5[0] == CC5_init

        CD2_inital, CD2[0] == CD2_init
        CD3_inital, CD3[0] == CD3_init
        CD4_inital, CD4[0] == CD4_init
        CD5_inital, CD5[0] == CD5_init

    end

    @NLconstraints mpc begin

        dV2_dt[k=0:Nd-1], V2[k] + (fix.F3[k+1] + F4[k] - F5[k]) * dtd == V2[k+1]

        dT2_dt[k=0:Nd-1], T2[k] + (
            (Q2[k] + F4[k] * CB0 * H_B(TB0) +
             (fix.F3[k+1] * fix.CA1[k+1] * H_A(fix.T1[k+1]) - F5[k] * CA2[k] * H_A(T2[k])) +
             (fix.F3[k+1] * fix.CB1[k+1] * H_B(fix.T1[k+1]) - F5[k] * CB2[k] * H_B(T2[k])) +
             (fix.F3[k+1] * fix.CC1[k+1] * H_C(fix.T1[k+1]) - F5[k] * CC2[k] * H_C(T2[k])) +
             (fix.F3[k+1] * fix.CD1[k+1] * H_D(fix.T1[k+1]) - F5[k] * CD2[k] * H_D(T2[k])))
            /
            (CA2[k] * Cp_A * V2[k] + CB2[k] * Cp_B * V2[k] + CC2[k] * Cp_C * V2[k] + CD2[k] * Cp_D * V2[k])
            +
            (-delH_r1 * r1(T2[k], CA2[k], CB2[k]) - delH_r2 * r2(T2[k], CB2[k], CC2[k], CD2[k]))
        ...
        dCA2_dt[k=0:Nd-1], CA2[k] + (((fix.F3[k+1] * fix.CA1[k+1] - F5[k] * CA2[k]) / V2[k]) - r1(T2[k], CA2[k], CB2[k])) * dtd == CA2[k+1]
        dCB2_dt[k=0:Nd-1], CB2[k] + ((fix.F3[k+1] * fix.CB1[k+1] + F4[k] * CB0 - F5[k] * CB2[k]) / V2[k] - r1(T2[k], CA2[k], CB2[k]) - r2(T2[k], CB2[k], CC2[k], CD2[k])) * dtd == CB2[k+1]
        dCC2_dt[k=0:Nd-1], CC2[k] + ((fix.F3[k+1] * fix.CC1[k+1] - F5[k] * CC2[k]) / V2[k] + r1(T2[k], CA2[k], CB2[k]) - r2(T2[k], CB2[k], CC2[k], CD2[k])) * dtd == CC2[k+1]
        dCD2_dt[k=0:Nd-1], CD2[k] + ((fix.F3[k+1] * fix.CD1[k+1] - F5[k] * CD2[k]) / V2[k] + r2(T2[k], CB2[k], CC2[k], CD2[k])) * dtd == CD2[k+1]

        dV3_dt[k=0:Nd-1], V3[k] + (F5[k] + F6[k] - F7[k]) * dtd == V3[k+1]
        ...
        dCA3_dt[k=0:Nd-1], CA3[k] + (((F5[k] * CA2[k] - F7[k] * CA3[k]) / V3[k]) - r1(T3[k], CA3[k], CB3[k])) * dtd == CA3[k+1]
        dCB3_dt[k=0:Nd-1], CB3[k] + (((F5[k] * CB2[k] + F6[k] * CB0 - F7[k] * CB3[k]) / V3[k]) - r1(T3[k], CA3[k], CB3[k]) - r2(T3[k], CB3[k], CC3[k], CD3[k])) * dtd == CB3[k+1]
        dCC3_dt[k=0:Nd-1], CC3[k] + ((F5[k] * CC2[k] - F7[k] * CC3[k]) / V3[k] + r1(T3[k], CA3[k], CB3[k]) - r2(T3[k], CB3[k], CC3[k], CD3[k])) * dtd == CC3[k+1]
        dCD3_dt[k=0:Nd-1], CD3[k] + ((F5[k] * CD2[k] - F7[k] * CD3[k]) / V3[k] + r2(T3[k], CB3[k], CC3[k], CD3[k])) * dtd == CD3[k+1]

        dV4_dt[k=0:Nd-1], V4[k] + (F7[k] + F9[k] - F8[k] - Fr1[k] - Fr2[k]) * dtd == V4[k+1]
        ...
        dCA4_dt[k=0:Nd-1], CA4[k] + ((F7[k] * CA3[k] + F9[k] * CA5[k] - (Fr1[k] + Fr2[k]) * CAr(MA(T4[k], F7[k], CA3[k], F9[k], CA5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MB(T4[k], F7[k], CB3[k], F9[k], CB5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MC(T4[k], F7[k], CC3[k], F9[k], CC5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MD(T4[k], F7[k], CD3[k], F9[k], CD5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k])) - F8[k] * CA4[k]) / V4[k]) * dtd == CA4[k+1]
        dCB4_dt[k=0:Nd-1], CB4[k] + ((F7[k] * CB3[k] + F9[k] * CB5[k] - (Fr1[k] + Fr2[k]) * CBr(MA(T4[k], F7[k], CA3[k], F9[k], CA5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MB(T4[k], F7[k], CB3[k], F9[k], CB5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MC(T4[k], F7[k], CC3[k], F9[k], CC5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MD(T4[k], F7[k], CD3[k], F9[k], CD5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k])) - F8[k] * CB4[k]) / V4[k]) * dtd == CB4[k+1]
        dCC4_dt[k=0:Nd-1], CC4[k] + ((F7[k] * CC3[k] + F9[k] * CC5[k] - (Fr1[k] + Fr2[k]) * CCr(MA(T4[k], F7[k], CA3[k], F9[k], CA5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MB(T4[k], F7[k], CB3[k], F9[k], CB5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MC(T4[k], F7[k], CC3[k], F9[k], CC5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MD(T4[k], F7[k], CD3[k], F9[k], CD5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k])) - F8[k] * CC4[k]) / V4[k]) * dtd == CC4[k+1]
        dCD4_dt[k=0:Nd-1], CD4[k] + ((F7[k] * CD3[k] + F9[k] * CD5[k] - (Fr1[k] + Fr2[k]) * CDr(MA(T4[k], F7[k], CA3[k], F9[k], CA5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MB(T4[k], F7[k], CB3[k], F9[k], CB5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MC(T4[k], F7[k], CC3[k], F9[k], CC5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MD(T4[k], F7[k], CD3[k], F9[k], CD5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k])) - F8[k] * CD4[k]) / V4[k]) * dtd == CD4[k+1]

        dV5_dt[k=0:Nd-1], V5[k] + (F10[k] + Fr1[k] - F9[k]) * dtd == V5[k+1]
        ...
        dCA5_dt[k=0:Nd-1], CA5[k] + ((Fr1[k] * CAr(MA(T4[k], F7[k], CA3[k], F9[k], CA5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MB(T4[k], F7[k], CB3[k], F9[k], CB5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MC(T4[k], F7[k], CC3[k], F9[k], CC5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MD(T4[k], F7[k], CD3[k], F9[k], CD5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k])) - F9[k] * CA5[k]) / V5[k] - r3(T5[k], CA5[k], CD5[k])) * dtd == CA5[k+1]
        dCB5_dt[k=0:Nd-1], CB5[k] + ((Fr1[k] * CBr(MA(T4[k], F7[k], CA3[k], F9[k], CA5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MB(T4[k], F7[k], CB3[k], F9[k], CB5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MC(T4[k], F7[k], CC3[k], F9[k], CC5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MD(T4[k], F7[k], CD3[k], F9[k], CD5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k])) - F9[k] * CB5[k]) / V5[k] - r2(T5[k], CB5[k], CC5[k], CD5[k])) * dtd == CB5[k+1]
        dCC5_dt[k=0:Nd-1], CC5[k] + ((Fr1[k] * CCr(MA(T4[k], F7[k], CA3[k], F9[k], CA5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MB(T4[k], F7[k], CB3[k], F9[k], CB5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MC(T4[k], F7[k], CC3[k], F9[k], CC5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MD(T4[k], F7[k], CD3[k], F9[k], CD5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k])) - F9[k] * CC5[k]) / V5[k] - r2(T5[k], CB5[k], CC5[k], CD5[k]) + 2 * r3(T5[k], CA5[k], CD5[k])) * dtd == CC5[k+1]
        dCD5_dt[k=0:Nd-1], CD5[k] + ((Fr1[k] * CDr(MA(T4[k], F7[k], CA3[k], F9[k], CA5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MB(T4[k], F7[k], CB3[k], F9[k], CB5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MC(T4[k], F7[k], CC3[k], F9[k], CC5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k]), MD(T4[k], F7[k], CD3[k], F9[k], CD5[k], CA3[k], CB3[k], CC3[k], CD3[k], CA5[k], CB5[k], CC5[k], CD5[k])) + F10[k] * CD0 - F9[k] * CD5[k]) / V5[k] + r2(T5[k], CB5[k], CC5[k], CD5[k]) - r3(T5[k], CA5[k], CD5[k])) * dtd == CD5[k+1]
    ...
    @NLobjective(mpc, Min, sum(
        w.v * (V2[k] - V2_sp)^2 + w.v * (V3[k] - V3_sp)^2 + w.v * (V4[k] - V4_sp)^2 + w.v * (V5[k] - V5_sp)^2 +
        w.t2 * (T2[k] - T2_sp)^2 + w.t3 * (T3[k] - T3_sp)^2 + w.t4 * (T4[k] - T4_sp)^2 + w.t5 * (T5[k] - T5_sp)^2 +
        w.ca2 * (CA2[k] - CA2_sp)^2 + w.ca3 * (CA3[k] - CA3_sp)^2 + w.ca4 * (CA4[k] - CA4_sp)^2 + w.ca5 * (CA5[k] - CA5_sp)^2 +
        w.cb2 * (CB2[k] - CB2_sp)^2 + w.cb3 * (CB3[k] - CB3_sp)^2 + w.cb4 * (CB4[k] - CB4_sp)^2 + w.cb5 * (CB5[k] - CB5_sp)^2 +
        w.cc2 * (CC2[k] - CC2_sp)^2 + w.cc3 * (CC3[k] - CC3_sp)^2 + w.cc4 * (CC4[k] - CC4_sp)^2 + w.cc5 * (CC5[k] - CC5_sp)^2 +
        w.cd2 * (CD2[k] - CD2_sp)^2 + w.cd3 * (CD3[k] - CD3_sp)^2 + w.cd4 * (CD4[k] - CD4_sp)^2 + w.cd5 * (CD5[k] - CD5_sp)^2
        for k = 0:Nd) +
                           sum(
        w.f4 * (F4[k] - F4_sp)^2 + w.f5 * (F5[k] - F5_sp)^2 + w.f6 * (F6[k] - F6_sp)^2 + w.f7 * (F7[k] - F7_sp)^2 + w.f8 * (F8[k] - F8_sp)^2 + w.f9 * (F9[k] - F9_sp)^2 + w.f10 * (F10[k] - F10_sp)^2
        + w.fr1 * (Fr1[k] - Fr1_sp)^2 + w.fr2 * (Fr2[k] - Fr2_sp)^2
        + w.q2 * (Q2[k] - Q2_sp)^2 + w.q3 * (Q3[k] - Q3_sp)^2 + w.q4 * (Q4[k] - Q4_sp)^2 + w.q5 * (Q5[k] - Q5_sp)^2 for k = 0:Nd
    )
    )
    ...
    return F4, F5, F6, F7, F8, F9, F10, Fr1, Fr2, Q2, Q3, Q4, Q5, V2, V3, V4, V5, T2, T3, T4, T5, CA2, CA3, CA4, CA5, CB2, CB3, CB4, CB5, CC2, CC3, CC4, CC5, CD2, CD3, CD4, CD5
end

function dmpc_1_arc2()

    # Control variables: F9, F10, Fr1, Fr2, Q5
    # State variables: All state variables associated with CSTR-5
    ...
    @NLconstraints mpc begin
        ...
        dT5_dt[k=0:Nd-1], T5[k] + (
            ((Q5[k] +
              F10[k] * CD0 * H_D(TD0)
              + (Fr1[k] * CAr(MA(fix.T4[k+1], fix.F7[k+1], fix.CA3[k+1], F9[k], CA5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MB(fix.T4[k+1], fix.F7[k+1], fix.CB3[k+1], F9[k], CB5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MC(fix.T4[k+1], fix.F7[k+1], fix.CC3[k+1], F9[k], CC5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MD(fix.T4[k+1], fix.F7[k+1], fix.CD3[k+1], F9[k], CD5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k])) * H_A(fix.T4[k+1]) - F9[k] * CA5[k] * H_A(T5[k]))
              + (Fr1[k] * CBr(MA(fix.T4[k+1], fix.F7[k+1], fix.CA3[k+1], F9[k], CA5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MB(fix.T4[k+1], fix.F7[k+1], fix.CB3[k+1], F9[k], CB5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MC(fix.T4[k+1], fix.F7[k+1], fix.CC3[k+1], F9[k], CC5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MD(fix.T4[k+1], fix.F7[k+1], fix.CD3[k+1], F9[k], CD5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k])) * H_B(fix.T4[k+1]) - F9[k] * CB5[k] * H_B(T5[k]))
              + (Fr1[k] * CCr(MA(fix.T4[k+1], fix.F7[k+1], fix.CA3[k+1], F9[k], CA5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MB(fix.T4[k+1], fix.F7[k+1], fix.CB3[k+1], F9[k], CB5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MC(fix.T4[k+1], fix.F7[k+1], fix.CC3[k+1], F9[k], CC5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MD(fix.T4[k+1], fix.F7[k+1], fix.CD3[k+1], F9[k], CD5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k])) * H_C(fix.T4[k+1]) - F9[k] * CC5[k] * H_C(T5[k]))
              + (Fr1[k] * CDr(MA(fix.T4[k+1], fix.F7[k+1], fix.CA3[k+1], F9[k], CA5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MB(fix.T4[k+1], fix.F7[k+1], fix.CB3[k+1], F9[k], CB5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MC(fix.T4[k+1], fix.F7[k+1], fix.CC3[k+1], F9[k], CC5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MD(fix.T4[k+1], fix.F7[k+1], fix.CD3[k+1], F9[k], CD5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k])) * H_D(fix.T4[k+1]) - F9[k] * CD5[k] * H_D(T5[k])))
             /
             (CA5[k] * Cp_A * V5[k] + CB5[k] * Cp_B * V5[k] + CC5[k] * Cp_C * V5[k] + CD5[k] * Cp_D * V5[k]))
            +
            ((-delH_r2 * r2(T5[k], CB5[k], CC5[k], CD5[k]) - delH_r3 * r3(T5[k], CA5[k], CD5[k]))
        ...
        dCA5_dt[k=0:Nd-1], CA5[k] + ((Fr1[k] * CAr(MA(fix.T4[k+1], fix.F7[k+1], fix.CA3[k+1], F9[k], CA5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MB(fix.T4[k+1], fix.F7[k+1], fix.CB3[k+1], F9[k], CB5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MC(fix.T4[k+1], fix.F7[k+1], fix.CC3[k+1], F9[k], CC5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MD(fix.T4[k+1], fix.F7[k+1], fix.CD3[k+1], F9[k], CD5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k])) - F9[k] * CA5[k]) / V5[k] - r3(T5[k], CA5[k], CD5[k])) * dtd == CA5[k+1]
        dCB5_dt[k=0:Nd-1], CB5[k] + ((Fr1[k] * CBr(MA(fix.T4[k+1], fix.F7[k+1], fix.CA3[k+1], F9[k], CA5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MB(fix.T4[k+1], fix.F7[k+1], fix.CB3[k+1], F9[k], CB5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MC(fix.T4[k+1], fix.F7[k+1], fix.CC3[k+1], F9[k], CC5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MD(fix.T4[k+1], fix.F7[k+1], fix.CD3[k+1], F9[k], CD5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k])) - F9[k] * CB5[k]) / V5[k] - r2(T5[k], CB5[k], CC5[k], CD5[k])) * dtd == CB5[k+1]
        dCC5_dt[k=0:Nd-1], CC5[k] + ((Fr1[k] * CCr(MA(fix.T4[k+1], fix.F7[k+1], fix.CA3[k+1], F9[k], CA5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MB(fix.T4[k+1], fix.F7[k+1], fix.CB3[k+1], F9[k], CB5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MC(fix.T4[k+1], fix.F7[k+1], fix.CC3[k+1], F9[k], CC5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MD(fix.T4[k+1], fix.F7[k+1], fix.CD3[k+1], F9[k], CD5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k])) - F9[k] * CC5[k]) / V5[k] - r2(T5[k], CB5[k], CC5[k], CD5[k]) + 2 * r3(T5[k], CA5[k], CD5[k])) * dtd == CC5[k+1]
        dCD5_dt[k=0:Nd-1], CD5[k] + ((Fr1[k] * CDr(MA(fix.T4[k+1], fix.F7[k+1], fix.CA3[k+1], F9[k], CA5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MB(fix.T4[k+1], fix.F7[k+1], fix.CB3[k+1], F9[k], CB5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MC(fix.T4[k+1], fix.F7[k+1], fix.CC3[k+1], F9[k], CC5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k]), MD(fix.T4[k+1], fix.F7[k+1], fix.CD3[k+1], F9[k], CD5[k], fix.CA3[k+1], fix.CB3[k+1], fix.CC3[k+1], fix.CD3[k+1], CA5[k], CB5[k], CC5[k], CD5[k])) + F10[k] * CD0 - F9[k] * CD5[k]) / V5[k] + r2(T5[k], CB5[k], CC5[k], CD5[k]) - r3(T5[k], CA5[k], CD5[k])) * dtd == CD5[k+1]
    ...
    @NLobjective(mpc, Min, sum(
        w.v * (V5[k] - V5_sp)^2 +
        w.t5 * (T5[k] - T5_sp)^2 +
        w.ca5 * (CA5[k] - CA5_sp)^2 +
        w.cb5 * (CB5[k] - CB5_sp)^2 +
        w.cc5 * (CC5[k] - CC5_sp)^2 +
        w.cd5 * (CD5[k] - CD5_sp)^2
        for k = 0:Nd) +
                           sum(
        w.f9 * (F9[k] - F9_sp)^2 + w.f10 * (F10[k] - F10_sp)^2 + w.fr1 * (Fr1[k] - Fr1_sp)^2 + w.fr2 * (Fr2[k] - Fr2_sp)^2 +
        w.q5 * (Q5[k] - Q5_sp)^2 for k = 0:Nd
    )
    )
    ...
    return F9, F10, Fr1, Fr2, Q5, V5, T5, CA5, CB5, CC5, CD5
end

function dmpc_2_arc2()
    ...
    @constraints mpc begin
        ...
        CA1_inital, CA1[0] == CA1_init
        CA2_inital, CA2[0] == CA2_init
        CA3_inital, CA3[0] == CA3_init
        CA4_inital, CA4[0] == CA4_init

        CB1_inital, CB1[0] == CB1_init
        CB2_inital, CB2[0] == CB2_init
        CB3_inital, CB3[0] == CB3_init
        CB4_inital, CB4[0] == CB4_init

        CC1_inital, CC1[0] == CC1_init
        CC2_inital, CC2[0] == CC2_init
        CC3_inital, CC3[0] == CC3_init
        CC4_inital, CC4[0] == CC4_init

        CD1_inital, CD1[0] == CD1_init
        CD2_inital, CD2[0] == CD2_init
        CD3_inital, CD3[0] == CD3_init
        CD4_inital, CD4[0] == CD4_init
    ...
    @NLconstraints mpc begin
        ...
        dT1_dt[k=0:Nd-1], T1[k] + (
            ((Q1[k] + F1[k] * CA0 * H_A(TA0) + F2[k] * CB0 * H_B(TB0) +
              (fix.Fr2[k+1] * CAr(MA(T4[k], F7[k], CA3[k], fix.F9[k+1], fix.CA5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MB(T4[k], F7[k], CB3[k], fix.F9[k+1], fix.CB5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MC(T4[k], F7[k], CC3[k], fix.F9[k+1], fix.CC5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MD(T4[k], F7[k], CD3[k], fix.F9[k+1], fix.CD5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1])) * H_A(T4[k]) - F3[k] * CA1[k] * H_A(T1[k])) +
              (fix.Fr2[k+1] * CBr(MA(T4[k], F7[k], CA3[k], fix.F9[k+1], fix.CA5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MB(T4[k], F7[k], CB3[k], fix.F9[k+1], fix.CB5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MC(T4[k], F7[k], CC3[k], fix.F9[k+1], fix.CC5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MD(T4[k], F7[k], CD3[k], fix.F9[k+1], fix.CD5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1])) * H_B(T4[k]) - F3[k] * CB1[k] * H_B(T1[k])) +
              (fix.Fr2[k+1] * CCr(MA(T4[k], F7[k], CA3[k], fix.F9[k+1], fix.CA5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MB(T4[k], F7[k], CB3[k], fix.F9[k+1], fix.CB5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MC(T4[k], F7[k], CC3[k], fix.F9[k+1], fix.CC5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MD(T4[k], F7[k], CD3[k], fix.F9[k+1], fix.CD5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1])) * H_C(T4[k]) - F3[k] * CC1[k] * H_C(T1[k])) +
              (fix.Fr2[k+1] * CDr(MA(T4[k], F7[k], CA3[k], fix.F9[k+1], fix.CA5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MB(T4[k], F7[k], CB3[k], fix.F9[k+1], fix.CB5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MC(T4[k], F7[k], CC3[k], fix.F9[k+1], fix.CC5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MD(T4[k], F7[k], CD3[k], fix.F9[k+1], fix.CD5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1])) * H_D(T4[k]) - F3[k] * CD1[k] * H_D(T1[k])))
             /
             (CA1[k] * Cp_A * V1[k] + CB1[k] * Cp_B * V1[k] + CC1[k] * Cp_C * V1[k] + CD1[k] * Cp_D * V1[k])) +
            (-delH_r1 * r1(T1[k], CA1[k], CB1[k]) - delH_r2 * r2(T1[k], CB1[k], CC1[k], CD1[k])) /
            (CA1[k] * Cp_A + CB1[k] * Cp_B + CC1[k] * Cp_C + CD1[k] * Cp_D)) * dtd == T1[k+1]
        ...
        dCA1_dt[k=0:Nd-1], CA1[k] + (
            ((F1[k] * CA0 + fix.Fr2[k+1] * CAr(MA(T4[k], F7[k], CA3[k], fix.F9[k+1], fix.CA5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MB(T4[k], F7[k], CB3[k], fix.F9[k+1], fix.CB5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MC(T4[k], F7[k], CC3[k], fix.F9[k+1], fix.CC5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]), MD(T4[k], F7[k], CD3[k], fix.F9[k+1], fix.CD5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]))
              -
              F3[k] * CA1[k])
             /
             V1[k]) - r1(T1[k], CA1[k], CB1[k])) * dtd == CA1[k+1]
        ...
        dV2_dt[k=0:Nd-1], V2[k] + (F3[k] + F4[k] - F5[k]) * dtd == V2[k+1]
        ...
        dCA2_dt[k=0:Nd-1], CA2[k] + (((F3[k] * CA1[k] - F5[k] * CA2[k]) / V2[k]) - r1(T2[k], CA2[k], CB2[k])) * dtd == CA2[k+1]
        dCB2_dt[k=0:Nd-1], CB2[k] + ((F3[k] * CB1[k] + F4[k] * CB0 - F5[k] * CB2[k]) / V2[k] - r1(T2[k], CA2[k], CB2[k]) - r2(T2[k], CB2[k], CC2[k], CD2[k])) * dtd == CB2[k+1]
        dCC2_dt[k=0:Nd-1], CC2[k] + ((F3[k] * CC1[k] - F5[k] * CC2[k]) / V2[k] + r1(T2[k], CA2[k], CB2[k]) - r2(T2[k], CB2[k], CC2[k], CD2[k])) * dtd == CC2[k+1]
        dCD2_dt[k=0:Nd-1], CD2[k] + ((F3[k] * CD1[k] - F5[k] * CD2[k]) / V2[k] + r2(T2[k], CB2[k], CC2[k], CD2[k])) * dtd == CD2[k+1]
        ...
        dV4_dt[k=0:Nd-1], V4[k] + (F7[k] + fix.F9[k+1] - F8[k] - fix.Fr1[k+1] - fix.Fr2[k+1]) * dtd == V4[k+1]

        dT4_dt[k=0:Nd-1], T4[k] +
                          ((Q4[k]
                            + (F7[k] * CA3[k] * H_A(T3[k]) + fix.F9[k+1] * fix.CA5[k+1] * H_A(fix.T5[k+1]) - MA(T4[k], F7[k], CA3[k], fix.F9[k+1], fix.CA5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]) * H_A(T4[k]) - F8[k] * CA4[k] * H_A(T4[k]) - MA(T4[k], F7[k], CA3[k], fix.F9[k+1], fix.CA5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]) * H_vap_A)
                            + (F7[k] * CB3[k] * H_B(T3[k]) + fix.F9[k+1] * fix.CB5[k+1] * H_B(fix.T5[k+1]) - MB(T4[k], F7[k], CB3[k], fix.F9[k+1], fix.CB5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]) * H_B(T4[k]) - F8[k] * CB4[k] * H_B(T4[k]) - MB(T4[k], F7[k], CB3[k], fix.F9[k+1], fix.CB5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]) * H_vap_B)
                            + (F7[k] * CC3[k] * H_C(T3[k]) + fix.F9[k+1] * fix.CC5[k+1] * H_C(fix.T5[k+1]) - MC(T4[k], F7[k], CC3[k], fix.F9[k+1], fix.CC5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]) * H_C(T4[k]) - F8[k] * CC4[k] * H_C(T4[k]) - MC(T4[k], F7[k], CC3[k], fix.F9[k+1], fix.CC5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]) * H_vap_C)
                            + (F7[k] * CD3[k] * H_D(T3[k]) + fix.F9[k+1] * fix.CD5[k+1] * H_D(fix.T5[k+1]) - MD(T4[k], F7[k], CD3[k], fix.F9[k+1], fix.CD5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]) * H_D(T4[k]) - F8[k] * CD4[k] * H_D(T4[k]) - MD(T4[k], F7[k], CD3[k], fix.F9[k+1], fix.CD5[k+1], CA3[k], CB3[k], CC3[k], CD3[k], fix.CA5[k+1], fix.CB5[k+1], fix.CC5[k+1], fix.CD5[k+1]) * H_vap_D))
                           /
                           (CA4[k] * Cp_A * V4[k] + CB4[k] * Cp_B * V4[k] + CC4[k] * Cp_C * V4[k] + CD4[k] * Cp_D * V4[k])) * dtd == T4[k+1]
    ...
    @NLobjective(mpc, Min, sum(
        w.v * (V1[k] - V1_sp)^2 + w.v * (V2[k] - V2_sp)^2 + w.v * (V3[k] - V3_sp)^2 + w.v * (V4[k] - V4_sp)^2 +
        w.t1 * (T1[k] - T1_sp)^2 + w.t2 * (T2[k] - T2_sp)^2 + w.t3 * (T3[k] - T3_sp)^2 + w.t4 * (T4[k] - T4_sp)^2 +
        w.ca1 * (CA1[k] - CA1_sp)^2 + w.ca2 * (CA2[k] - CA2_sp)^2 + w.ca3 * (CA3[k] - CA3_sp)^2 + w.ca4 * (CA4[k] - CA4_sp)^2 +
        w.cb1 * (CB1[k] - CB1_sp)^2 + w.cb2 * (CB2[k] - CB2_sp)^2 + w.cb3 * (CB3[k] - CB3_sp)^2 + w.cb4 * (CB4[k] - CB4_sp)^2 +
        w.cc1 * (CC1[k] - CC1_sp)^2 + w.cc2 * (CC2[k] - CC2_sp)^2 + w.cc3 * (CC3[k] - CC3_sp)^2 + w.cc4 * (CC4[k] - CC4_sp)^2 +
        w.cd1 * (CD1[k] - CD1_sp)^2 + w.cd2 * (CD2[k] - CD2_sp)^2 + w.cd3 * (CD3[k] - CD3_sp)^2 + w.cd4 * (CD4[k] - CD4_sp)^2
        for k = 0:Nd) +
                           sum(
        w.f1 * (F1[k] - F1_sp)^2 + w.f2 * (F2[k] - F2_sp)^2 + w.f3 * (F3[k] - F3_sp)^2 + w.f4 * (F4[k] - F4_sp)^2 +
        w.f5 * (F5[k] - F5_sp)^2 + w.f6 * (F6[k] - F6_sp)^2 + w.f7 * (F7[k] - F7_sp)^2 + w.f8 * (F8[k] - F8_sp)^2 +
        w.q1 * (Q1[k] - Q1_sp)^2 + w.q2 * (Q2[k] - Q2_sp)^2 + w.q3 * (Q3[k] - Q3_sp)^2 + w.q4 * (Q4[k] - Q4_sp)^2 for k = 0:Nd
    )
    )
    ...
    V1 = Vector(JuMP.value.(V1))
    V2 = Vector(JuMP.value.(V2))
    V3 = Vector(JuMP.value.(V1))
    V4 = Vector(JuMP.value.(V2))
    ...
    return F1, F2, F3, F4, F5, F6, F7, F8, Q1, Q2, Q3, Q4, V1, V2, V3, V4, T1, T2, T3, T4, CA1, CA2, CA3, CA4, CB1, CB2, CB3, CB4, CC1, CC2, CC3, CC4, CD1, CD2, CD3, CD4
end

function dmpc_1_arc3()
    ...
    @NLconstraints mpc begin
        ...
        # volHoldUp1[k=0:Nd-1], F1[k] + F2[k] + fix.Fr2[k+1] - F3[k] == -(V1[k] - V1_sp) / 10
        # volHoldUp2[k=0:Nd-1], F3[k] + F4[k] - fix.F5[k+1] == -(V2[k] - V2_sp) / 10
...
function dmpc_2_arc3()
    ...
    @NLconstraints mpc begin
        ...
        # volHoldUp3[k=0:Nd-1], F5[k] + F6[k] - fix.F7[k+1] == -(V3[k] - V3_sp) / 10
...
function dmpc_3_arc3()
    ...
    @NLconstraints mpc begin
        ...
        # volHoldUp4[k=0:Nd-1], F7[k] + F9[k] - F8[k] - Fr1[k] - Fr2[k] == -(V4[k] - V4_sp) / 10
        # volHoldUp5[k=0:Nd-1], F10[k] + Fr1[k] - F9[k] == -(V5[k] - V5_sp) / 10
...
function dmpc(architecture)

    max_steps = 15
    global dmpc_solve_time = 0
    for steps = 1:max_steps
        if architecture == 1
            F1, F2, F3, Q1, V1, T1, CA1, CB1, CC1, CD1 = dmpc_1_arc1()
            F4, F5, F6, F7, F8, F9, F10, Fr1, Fr2, Q2, Q3, Q4, Q5, V2, V3, V4, V5, T2, T3, T4, T5, CA2, CA3, CA4, CA5, CB2, CB3, CB4, CB5, CC2, CC3, CC4, CC5, CD2, CD3, CD4, CD5 = dmpc_2_arc1()
        end
        if architecture == 2
            F9, F10, Fr1, Fr2, Q5, V5, T5, CA5, CB5, CC5, CD5 = dmpc_1_arc2()
            F1, F2, F3, F4, F5, F6, F7, F8, Q1, Q2, Q3, Q4, V1, V2, V3, V4, T1, T2, T3, T4, CA1, CA2, CA3, CA4, CB1, CB2, CB3, CB4, CC1, CC2, CC3, CC4, CD1, CD2, CD3, CD4 = dmpc_2_arc2()
        end
        if architecture == 3
            F1, F2, F3, F4, Q1, Q2, V1, V2, T1, T2, CA1, CA2, CB1, CB2, CC1, CC2, CD1, CD2 = dmpc_1_arc3()
            F5, F6, Q3, V3, T3, CA3, CB3, CC3, CD3 = dmpc_2_arc3()
            F7, F8, F9, F10, Fr1, Fr2, Q4, Q5, V4, V5, T4, T5, CA4, CA5, CB4, CB5, CC4, CC5, CD4, CD5 = dmpc_3_arc3()
        end
...
dentr_ISE, dentr_ISC, dentr_PI = getPI(dF1, dF2, dF3, dF4, dF5, dF6, dF7, dF8, dF9, dF10, dFr1, dFr2, dQ1, dQ2, dQ3, dQ4, dQ5)
...
p1 = plot(CA2, label=false)
hline!([CA2_sp], label=false)

p2 = plot(dF2, label=false)
hline!([F2_sp], label=false)

global dF1, dF2, dF3, dF4, dF5, dF6, dF7, dF8, dF9, dF10, dFr1, dFr2, dQ1, dQ2, dQ3, dQ4, dQ5 = dmpc(2)
dentr_ISE, dentr_ISC, dentr_PI = getPI(dF1, dF2, dF3, dF4, dF5, dF6, dF7, dF8, dF9, dF10, dFr1, dFr2, dQ1, dQ2, dQ3, dQ4, dQ5)
V1, V2, V3, V4, V5, T1, T2, T3, T4, T5, CA1, CA2, CA3, CA4, CA5, CB1, CB2, CB3, CB4, CB5, CC1, CC2, CC3, CC4, CC5, CD1, CD2, CD3, CD4, CD5 = getTraj(dF1, dF2, dF3, dF4, dF5, dF6, dF7, dF8, dF9, dF10, dFr1, dFr2, dQ1, dQ2, dQ3, dQ4, dQ5)
...
p3 = plot(CA2, label=false)
hline!([CA2_sp], label=false)

p4 = plot(dF2, label=false)
hline!([F2_sp], label=false)

global dF1, dF2, dF3, dF4, dF5, dF6, dF7, dF8, dF9, dF10, dFr1, dFr2, dQ1, dQ2, dQ3, dQ4, dQ5 = dmpc(3)
dentr_ISE, dentr_ISC, dentr_PI = getPI(dF1, dF2, dF3, dF4, dF5, dF6, dF7, dF8, dF9, dF10, dFr1, dFr2, dQ1, dQ2, dQ3, dQ4, dQ5)
V1, V2, V3, V4, V5, T1, T2, T3, T4, T5, CA1, CA2, CA3, CA4, CA5, CB1, CB2, CB3, CB4, CB5, CC1, CC2, CC3, CC4, CC5, CD1, CD2, CD3, CD4, CD5 = getTraj(dF1, dF2, dF3, dF4, dF5, dF6, dF7, dF8, dF9, dF10, dFr1, dFr2, dQ1, dQ2, dQ3, dQ4, dQ5)
...
p5 = plot(CA2, label=false)
hline!([CA2_sp], label=false)

p6 = plot(dF2, label=false)
hline!([F2_sp], label=false)
